### Hi there / こんにちは

I'm **Yasufumi Nakata (中田康史)**, a **master's student at Keio University Graduate School of Media and Governance** and a member of **Aoyama Lab**.
My work sits across ** neuroscience-related research**.

慶應義塾大学大学院 政策・メディア研究科の修士課程に在籍し、**青山研究室**に所属しています。
関心領域は **経科学関連研究** です。

---

## About Me / 自己紹介

- **Current Focus / 現在の取り組み**
  - Building practical products and research prototypes around **AI**, **geospatial/location data**, and **web applications**
  - **EEG / VR** を含む人間の認知・行動理解に関する研究開発

- **Affiliations / 所属**
  - **Keio University Graduate School of Media and Governance**
  - **Aoyama Lab**
  - **WBA Initiative** Auditor / Secretariat

- **Selected Work / 主な取り組み**
  - **Location-based systems and map-oriented services**
  - **Behavior recognition** using multimodal signals and machine learning
  - **AI-assisted research tooling**, including paper and knowledge support workflows
  - **Frontend / Web development** for portfolio and product experiences

---

## Skills & Tools / スキル・使用ツール

- **Programming / プログラミング**
  - Python, JavaScript, HTML, CSS, MATLAB

- **Data / Research / 研究開発**
  -  EEG analysis, signal processing, data visualization

- **Tools / 使用ツール**
  - MNE-Python, EEGLAB, 
  - Git, Google Cloud

---

## Links / リンク

- **Website**: [yasufumi.net](https://yasufumi.net)
- **LinkedIn**: [yasufuminakata](https://www.linkedin.com/in/yasufuminakata/)
- **X**: [@Yasufumi_Nakata](https://x.com/Yasufumi_Nakata)
- **Google Scholar**: [Profile](https://scholar.google.co.jp/citations?hl=ja&user=GhMEe8wAAAAJ)
- **Email**: [yasufumi@keio.jp](mailto:yasufumi@keio.jp)

If you'd like to collaborate on AI, neuroscience, geospatial systems, or product development, feel free to reach out.
AI、神経科学、位置情報システム、プロダクト開発に関するご相談はお気軽にご連絡ください。
